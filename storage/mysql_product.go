...
// mySQLProduct used for work with postgres - product
...
func (p *mySQLProduct) Create(m *product.Model) error {
	...
	fmt.Printf("product created successfully with id: %v\n", id)
	return nil
}
...
func (p *mySQLProduct) Update(m *product.Model) error {
	...
	if rowsAffected != 1 {
		return fmt.Errorf("there is no product with id: %v", m.ID)
	}
...
func (p *mySQLProduct) Delete(id uint) error {
	...
	if rowsAffected != 1 {
		return fmt.Errorf("there is no product with id: %v", id)
	}
